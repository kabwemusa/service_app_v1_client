...
/**
 * § DB-8 — prune passive observability tables (WhatsApp webhook logs, PawaPay
...
class PruneObservabilityLogs extends Command
{
    ...
    public function handle(): int
    {
        ...
        $pawapay = DB::table('pawapay_events')->where('created_at', '<', $cutoff)->delete();

        if ($wa + $pawapay > 0) {
            $this->info("Pruned {$wa} webhook logs, {$pawapay} pawapay events (older than {$days}d).");
            Log::info('PruneObservabilityLogs: cycle complete', ['webhook_logs' => $wa, 'pawapay_events' => $pawapay, 'days' => $days]);
        }
